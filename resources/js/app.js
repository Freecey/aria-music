...
document.addEventListener('DOMContentLoaded', () => {
  ...
  Promise.all([
    ...
    .then(([site, albums, links, updates]) => {
      loadProgress = 100;
      ...
      setTimeout(() => {
        loader.hide();
        renderSiteData(site.data, albums.data, links.data, updates.data);
      }, 400);
    })
    .catch(err => {
  ...
  function renderSiteData(site, albums, links, updates) {
    // Update tagline phrases from API
    if (site.tagline) {
      const taglineEl = document.getElementById('tagline-text');
      if (taglineEl) {
        taglineEl.dataset.phrases = JSON.stringify([site.tagline]);
      }
    }
    ...
    if (albumsContainer && albums.length > 0) {
    ...
    if (updatesContainer && updates.length > 0) {
      updatesContainer.innerHTML = updates.map(update => `
        <div class="update-item">
          <p class="update-date">${new Date(update.published_at).toLocaleDateString('fr-FR')}</p>
    ...
    if (avatarEl && site.avatar_url) {
      avatarEl.src = site.avatar_url;
    }
    ...
    if (contactEmail) {
      ...
      if (emailLink) {
        ...
        contactEmail.innerHTML = `${emailLink.icon_svg || ''} ${emailLink.url.replace('mailto:', '')}`;
      }
    }
  }
